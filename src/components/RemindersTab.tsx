...
export default function RemindersTab({
  ...
  return (
    <div className="h-full bg-[#080808] font-sans text-slate-100 flex flex-col overflow-hidden relative">
      ...
      <div className="p-3 bg-[#0e0e0e]/95 border-b border-white/5 flex gap-2 justify-between items-center select-none shrink-0">
      ...
      {/* Checklist Timeline */}
      <div className="flex-1 overflow-y-auto p-4 space-y-3.5 pb-20 scrollbar-thin scrollbar-thumb-white/5">
        <AnimatePresence initial={false}>
          {filteredReminders.length === 0 ? (
            ...
            >
              ...
              <h4 className="font-medium text-xs text-slate-400">Không có mục nào</h4>
            ...
            filteredReminders.map(reminder => (
              ...
              >
                  ...
                  <div className="min-w-0 flex-1 space-y-1">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-[9px] text-slate-500">{getCategoryLabel(reminder.category)}</span>
                      </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <span className="text-[8px] font-mono font-bold uppercase tracking-wider bg-black border border-white/5 text-slate-500 py-0.5 px-2 rounded-full">
      ...
      <AnimatePresence>
        {isAdding && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 z-40 bg-black/95 backdrop-blur-md flex flex-col p-6 overflow-y-auto"
          >
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-sm font-medium text-slate-100 flex items-center gap-2">
                <Calendar className="w-5 h-5 text-[#c5a059]" />
                <span className="uppercase tracking-wider text-xs">Thêm Kế Hoạch Chung</span>
              </h3>
              <button
                onClick={() => setIsAdding(false)}
                className="p-1.5 rounded-full bg-white/5 border border-white/10 text-slate-400 hover:text-slate-100"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex-1 flex flex-col justify-between">
              <div className="space-y-5">
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-mono text-slate-400 uppercase">TÊN HOẠT ĐỘNG / KẾ HOẠCH</label>
                  <input
                    type="text"
                    required
                    value={title}
                ...
                {/* Category selectors */}
                <div className="space-y-1.5">
                  <label className="text-[10px] font-mono text-slate-400 uppercase">PHÂN LOẠI</label>
                  <div className="grid grid-cols-2 gap-2">
                    {([
                      ...
                      <button
                        ...
                        className={`p-3.5 rounded-xl border text-left flex items-center gap-2.5 transition-all text-xs ${
                ...
                {/* Due Date picker */}
                <div className="space-y-1.5">
                  <label className="text-[10px] font-mono text-slate-400 uppercase">HẠN HOÀN THÀNH</label>
                  <input
                    type="date"
                    required
                    value={dueDate}
                    onChange={e => setDueDate(e.target.value)}
                    className="w-full bg-[#121212] border border-white/5 rounded-xl py-3 px-4 text-xs text-slate-100 focus:outline-none focus:border-[#c5a059]/55"
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                className="w-full bg-[#c5a059] hover:bg-[#b08b47] text-black py-3 rounded-xl font-semibold text-xs shadow-md transition-all flex items-center justify-center gap-2"
              >
